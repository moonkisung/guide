    ...
    def generate_image(prompt, prompt2, negative_prompt, seed=None):
        ...
        with torch.no_grad():
            for i, t in enumerate(tqdm(timesteps)):
                ...
                noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_text - noise_pred_uncond) # guidance_scale는 5~7 이상 권장

                # compute the previous noisy sample x_t -> x_t-1
                # latents = scheduler.step(noise_pred, t, latents, **extra_step_kwargs).prev_sample
                step_result = scheduler.step(noise_pred, t, latents)
                latents = step_result.prev_sample
                #pred_original = step_result.pred_original_sample
                
            
                # latents = 1 / 0.18215 * latents
                latents_img = 1 / sdxl_model_util.VAE_SCALE_FACTOR * latents
                #latents_img = 1 / sdxl_model_util.VAE_SCALE_FACTOR * pred_original
            ...
            # timestamp = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
            # for j, img in enumerate(image):
            #     img.save(os.path.join(args.output_dir, f"image_.png"))
